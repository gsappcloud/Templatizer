...
gsappFetcher.parseEventBodyHTML = function(body_string) {
	console.log('!!!!!!!!!!!!!! beginning gsappFetcher.parseEventBodyHTML');
	console.log('body_string: ' + body_string);
	
	var idx = body_string.indexOf('<br');
	if(idx > 0){
		var idx2;
		var str2 = '';
		do{
			str2 = body_string.substring(idx);
			idx2 = str2.indexOf('<br');
			limit = idx+5;
			idx=idx2;
		}while( idx2 < limit );
		console.log('returning 2 paragraphs: ' + body_string.substring(0,idx) );
		
		body_string = body_string.substring(0,idx);
		
	}else{
		console.log('no br tags, returning full body_string');
	
	}
	
	var html_data = body_string.replace(/<[//]{0,1}(strong|b)[^><]*>/g,"");
	...
	try{
		var aIdx = body_string.indexOf('<a href="');
		if(aIdx > 0){
			var a1 = body_string.substring(0,aIdx);
			var a2 = body_string.substring(aIdx, 8);
			var a3 = body_string.substring(aIdx+8);
			var a3Idx = a3.indexOf('"');
			a3 = a3.substring(aIdx+8,a3Idx);
			var a4 = body_string.substring(a3Idx);
			body_string = a1+
		}
	
		body_element = $(body_string);
	}catch(e){
		console.log("ERROR: " + e.message);
		p_tags.push(body_string);
		return p_tags;
	}
	...
	for(var p=0;p<body_element.length;p++) {
		...
		//console.log('p: ' + p);
		//console.log('inner_element: ' + inner_element);
		//console.log('html_data: ' + inner_element.outerHTML);
		//console.log('inner_html: ' + inner_element.innerHTML);
		//console.log('.textContent: ' + inner_element.textContent);
			var html_data = inner_element.outerHTML;
			var inner_html = inner_element.innerHTML;
			var textContent = inner_element.textContent;
			// if inner_html len is < 6 then its probably just an nbsp tag
			if ((inner_html != undefined) && (inner_html.length > 6)) {
				// regex to strip out strong and b tags only
				html_data = html_data.replace(/<[//]{0,1}(strong|b)[^><]*>/g,"");
				p_tags.push(html_data);	
			}else if ((textContent != undefined) && (textContent.length > 6)) {
				p_tags.push(textContent);	
			}
		}
		if(html_data != undefined){
			if(html_data.substring(0,3) == '<br'){
				console.log('html_data: ' + html_data);
				para_count++;
				if(para_count >= 2){
					return p_tags;
...
gsappFetcher.getEventData = function(url, elementName) {
	...
	$.getJSON(url, function(data) {
		...
		for (var i=0; i<nodes.length;i++) {
			...
			var event_description_string = event_description;
